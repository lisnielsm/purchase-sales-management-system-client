...
export default {
	...
	methods: {
		async listar() {
			...
			try {
				const response = await axios.get("/api/usuarios/Listar", configuracion);
		...
		async select() {
			...
			try {
				const response = await axios.get("/api/roles/Select", configuracion);
		...
		async desactivar() {
			...
			try {
				await axios.put("api/usuarios/Desactivar/" + this.adId, {}, configuracion);
